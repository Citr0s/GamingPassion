		...
		<div id="container">
			...
			<div id="index-landing">
				...
				<?php
					...
					$user_data = mysql_query("SELECT * FROM `users` WHERE username = '$user'");
					...
					if(isset($_POST['current_password'])){
							if(!empty($current_password) && !empty($new_password) && !empty($new_password_check)){
								...
								$data4 = mysql_query("SELECT * FROM `users` WHERE password = '$current_password_check'");
						...
						if(empty($_POST['email'])){
							echo '
								...
								$data = mysql_query("SELECT * FROM `users` WHERE $user_id = user_id");
							...
							if(isset($_POST['delete_avatar'])){
								unlink($thumbnail);								
								$thumbnail = '';
								mysql_query("UPDATE `users` SET thumbnail = '$thumbnail' WHERE $user_id = user_id");	
							}
							...
							mysql_query("UPDATE `users` SET thumbnail = '$thumbnail', gender = '$gender', home = '$from', email = '$email' WHERE $user_id = user_id");	
								echo '<p><a href="profile.php?user='.$_SESSION['username'].'">< MY PROFILE</a></p>';
								echo '
					...
					while($row = mysql_fetch_array($data)){
						...
						echo '</td>
											...
											<tr>
												<td valign="middle" class="required">Sex:</td><td style="text-align:left;"><input type="radio" name="sex" value="male" style="width:10px; margin:5px;" '; if($gender == 'male'){echo 'checked';} echo ' >Male</td>
											</tr>
											<tr>
												<td></td><td style="text-align:left;"><input type="radio" name="sex" value="female" style="width:10px; margin:5px;" '; if($gender == 'female'){echo 'checked';} echo ' >Female</td>
											</tr>
											<tr>	
												<td></td><td style="text-align:left;"><input type="radio" name="sex" value="unknown" style="width:10px; margin:5px;" '; if($gender == 'unknown'){echo 'checked';} echo '>Don\'t Tell</td>
											</tr>
											<tr>
